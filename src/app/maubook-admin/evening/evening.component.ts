...
export class eveningComponent implements OnInit {
  packages: any[] = [];
  categories: string[] = [];
  ...
  loadPackages() {
    this.firestore.collection('packages').snapshotChanges().subscribe(
      (snapshot) => {
       ...
       this.packages =   this.packages.filter(pkg => pkg.category?.toLowerCase() === 'evening');
  ...
  addNewProduct() {
    this.router.navigate(['/add-evening']);
  }
  ...
  editPackage(id: string) {
    this.router.navigate(['/evening/edit', id]);
  }
